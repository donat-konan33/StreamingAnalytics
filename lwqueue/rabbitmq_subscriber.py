...
def make_live_callback(live_params: LiveParams):
    ...
    def live_callback(channel: str, method, properties, body):
        ...
        live_params["messages"].append(
            {**parsed_body, "received_at": str(datetime.datetime.utcnow())} # reconstitiion du message selon les exigences le la class MEssage et de LiveParams
        )
...
def receive_messages(
    ...
    result = channel.queue_declare(queue="display_messages", exclusive=True) # rmq fournira un nom de queue(amq.gen-xxxx) si queue="", le stocker permet juste de pouvoir le récupérer
